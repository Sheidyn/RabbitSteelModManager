...
public unsafe class FileAccessServer
{
    ...
    public static void Initialize(IReloadedHooks hooks, Redirector redirector, RedirectorController redirectorController, ILogger logger)
    {
        ...
        if (ntCreateFilePointer != IntPtr.Zero)
            _ntCreateFileHook = hooks.CreateHook<Native.NtCreateFile>(
                (delegate* unmanaged[Stdcall]<IntPtr*, FileAccess, Native.OBJECT_ATTRIBUTES*, Native.IO_STATUS_BLOCK*,
                    long*, uint, FileShare, uint, uint, IntPtr, uint, int>) &NtCreateFileHookFn,(long) ntCreateFilePointer).Activate();

        //if (ntQueryAttributesFilePointer != IntPtr.Zero)_ntQueryAttributesFileHook = hooks.CreateHook<Native.NtQueryAttributesFile>((delegate* unmanaged[Stdcall]<Native.OBJECT_ATTRIBUTES*, uint, int>) &NtQueryAttributesFileHookFn,(long) ntQueryAttributesFilePointer).Activate();
    }
        ...
        uint eaLength)
    {
        ...
        var newObjectPath = _prefix + newFilePath;
        //_logger.PrintMessage("NEW OBJECT PATH: " + newObjectPath, System.Drawing.Color.Green);
        fixed (char* address = newObjectPath)
        {
